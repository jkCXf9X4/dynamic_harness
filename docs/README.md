...
- `references/` — the durable rationale library loaded by the runtime's own
  ...
  live in the top-level `skills/` directory (see `../skills/` and the skills
...
guides all moved there. Task-specific instruction packages (skills) live in
[`../skills/`](../skills/), not in `docs/`.

## Why the split is intentional

`docs/references/` and `skills/` are loaded by the runtime's agent system
prompt, so they must stay at stable, runtime-known paths. `docs/api/` documents
