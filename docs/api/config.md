...
| `disable_root_timeout` | `true` | Exempt only the top (root) agent from `timeout_seconds`. The root runs until it finishes on its own; the person supervising decides when to kill it. Children still inherit the cap, so a stuck child force-fails and stays recoverable — a timeout is **never auto self-healed**; the parent decides via the `resume` tool (`strategy="resume"` same context / `"fresh"` clean retry) or by re-delegating. The per-call httpx timeout still bounds every request. |
...
| `references_dir` | `null` | Directory of durable, git-tracked reference docs (rationale, tool motivations, guidelines) that survive prompt optimization. A compact index is injected into every agent's environment; the agent reads full bodies on demand. Docs with `name` + `description` YAML frontmatter are treated as **skills** (optionally role-scoped via `roles`): their triggers are injected role-filtered into each agent's system-prompt block and loaded via the `skill_load` tool. Defaults to the harness package's own `docs/references` (resolved relative to the package, not the cwd, so it works from any project folder); falls back to a cwd-relative `docs/references` when the project carries its own library. |
